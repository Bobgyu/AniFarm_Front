  ...
  const handleDeleteComment = async (commentId) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        // DELETE 요청 전 로그
        console.log('삭제 요청 보내기:', commentId);
        ...
        if (response.data.success) {
          ...
          const refreshResponse = await axios.get('http://localhost:8000/api/comments/user', {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          // 새로 받아온 데이터 로그
          console.log('새로 받아온 댓글 데이터:', refreshResponse.data);

          if (refreshResponse.data.success) {
            setMyComments(refreshResponse.data.data);
            Swal.fire('삭제 완료', '댓글이 삭제되었습니다.', 'success');
          }
        } else {
          throw new Error('서버에서 삭제 실패 응답');
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.isConfirmed) {
        const response = await axios.put(
          `http://localhost:8000/api/comments/${comment.comment_id}`,
